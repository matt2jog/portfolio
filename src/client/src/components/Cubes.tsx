...
export interface CubesProps {
  ...
  borderStyle?: string;
  faceColor?: string;
  hoverFaceColor?: string;
  hoverFaceShadow?: string;
  hoverBorderColor?: string;
  shadow?: boolean | string;
  autoAnimate?: boolean;
  autoAnimateSpeed?: number;
  autoAnimatePause?: number;
...
const Cubes: React.FC<CubesProps> = ({
  ...
  const baseFaceShadow =
    shadow === true ? "0 0 6px rgba(0,0,0,.5)" : shadow || "none";
  const baseBorderColorMatch = borderStyle.match(/(rgba?\([^\)]+\)|#[0-9a-fA-F]{3,8})/);
  const baseBorderColor = baseBorderColorMatch?.[1];
      ...
      sceneRef.current.querySelectorAll<HTMLDivElement>(".cube").forEach((cube) => {
        ...
        if (dist <= radius) {
          ...
          gsap.to(faces, {
            ...
            ease: easing,
            overwrite: true,
            backgroundColor: hoverFaceColor ?? faceColor,
            boxShadow: hoverFaceShadow ?? baseFaceShadow,
  ...
  }, [leaveDur, faceColor, baseFaceShadow, baseBorderColor]);
  ...
  useEffect(() => {
    ...
    const loop = () => {
      if (!userActiveRef.current) {
        ...
        if (autoAnimatePause > 0 && pausedRef.current) {
        ...
        if (Math.hypot(pos.x - tgt.x, pos.y - tgt.y) < 0.1) {
          if (autoAnimatePause > 0) {
            pausedRef.current = true;
            pauseUntilRef.current = now + autoAnimatePause;
          } else {
            simTargetRef.current = {
              x: Math.random() * gridSize,
              y: Math.random() * gridSize,
  ...
  }, [autoAnimate, gridSize, tiltAt, autoAnimateSpeed, autoAnimatePause]);
  ...
  const wrapperStyle = {
    ...
    "--cube-face-shadow": baseFaceShadow,
    height: "100%",
    ...(cubeSize
      ? {
