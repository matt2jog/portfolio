...
async function databaseSessionEvidence(
  ...
  const result = await queryable.query(
    [
      ...
      "), vector_extension AS (",
      "  SELECT extension.oid, extension.extnamespace, namespace.nspname",
      ...
      "  COALESCE((SELECT has_schema_privilege(current_user, vector_extension.extnamespace, 'USAGE')",
      "    FROM vector_extension), false)",
      ...
      "            )",
      "          WHEN privilege.nspname = (SELECT nspname FROM vector_extension)",
      "            AND privilege.privilege_type = 'USAGE'",
      ...
      "  ELSE COALESCE((",
      "    SELECT count(*) = 2 AND bool_and(",
      "      privilege.nspname IN ('portfolio', (SELECT nspname FROM vector_extension))",
      "      AND privilege.privilege_type = 'USAGE'",
      "      AND NOT privilege.is_grantable",
      "    )",
      "    FROM schema_acl privilege",
      ...
      "      AND NOT (",
      "        privilege.nspname IN ('extensions', 'public')",
      "        AND privilege.privilege_type = 'USAGE'",
      "        AND NOT privilege.is_grantable",
      "        AND EXISTS (",
      "          SELECT 1 FROM pg_depend dependency",
      "          WHERE dependency.classid = 'pg_type'::regclass",
      "            AND dependency.objid = privilege.oid",
      "            AND dependency.refclassid = 'pg_extension'::regclass",
      "            AND dependency.refobjid = (SELECT oid FROM vector_extension)",
      "            AND dependency.deptype = 'e'",
      "        )",
