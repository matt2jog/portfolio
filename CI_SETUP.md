...
- `deploy.yml` runs on `main` pushes (plus manual dispatch from `main`). It relies on required PR CI for disposable Postgres migration/integration coverage and does not start localhost Postgres or export a local `DATABASE_URL`. It repeats secretless verification, builds the immutable merge-SHA image, scans before push, authenticates through the repository-bound `portfolio-main` Workload Identity provider, runs the actual migration bundle from that digest, and pushes only to `us-east4-docker.pkg.dev/personal-brand-501801/portfolio`.
...
- Secret Manager bundles: exact repository-variable-selected versions of `portfolio-runtime-bundle-prod`, `portfolio-deployment-bundle-prod`, and `portfolio-legal-audit-bundle-prod`, synchronized from Infisical by the root workspace workflow. Their database sources are respectively `PORTFOLIO_RUNTIME_DATABASE_URL` (`portfolio_runtime`), migration-only `PORTFOLIO_MIGRATION_DATABASE_URL`, and `PORTFOLIO_LEGAL_AUDIT_DATABASE_URL` (`legal_audit_writer`); every bundle also carries `PORTFOLIO_SUPABASE_PROJECT_REF` and the verified Supabase CA. Runtime and deployment contain the same generated `EDGE_ORIGIN_TOKEN`; during rotation both may also contain the immediately previous token until the rollback grace period ends. The deployment Cloudflare output is sourced only from `PORTFOLIO_CLOUDFLARE_API_TOKEN`. No human edits any copy in GCP or Cloudflare.
- Database-role prerequisite: `portfolio_runtime` must have only required application DML and no privileged-role inheritance, database CREATE, or public-schema CREATE; the migration URL is confined to the digest-pinned migration process and may hold the DDL ownership required by existing migrations; `legal_audit_writer` remains INSERT-only on `legal_document_versions`. Runtime and legal sessions verify their exact `session_user`/`current_user` and reject privileged attributes before work.
